...
for i in range(2000,4000+50,50):
    rho_r.append([float(i),float(1100),float(920)])
    
#print (rho_r) 
R=[]
for i in range(0,1700000,50000):
    R.append([float(i),float(1700000),float(1800000)])
...
for n in range(0,len(R)):
    for i in range(0,len(rho_r)):
        ...
        M = rho_r[i][1] * (pow(R[n][1],3)-pow(R[n][0],3))+rho_r[i][2] * (pow(R[n][2],3)-pow(R[n][1],3))
...
for n in range(0,len(R)):
    for i in range(0,len(rho_r)):
        ...
        C_1 = rho_r[i][1] * (pow(R[n][1],5)-pow(R[n][0],5)) + rho_r[i][2] * (pow(R[n][2],5)-pow(R[n][1],5))
        C.append([(C_1 + C_0)*(8/15.0)*math.pi, n, i])

MoI_computed=[]
for i in range(0,len(C)):
    MoI_computed.append(C[i][0]/(hmotnost[i][0]*pow(R[0][2],2)))
...
print(rho_r[hmotnost[index[0]][2]])
